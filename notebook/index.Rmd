...
> **This notebook tracks progress on the development of [resevol](https://bradduthie.github.io/resevol/) R package, a BBSRC [FAPESP Newton Funded](http://www.fapesp.br/en/9552) Project to model how the spatial scale of heterogeneity in fungal isolate application and crop plant cultivation affects variation in the selection landscape for biopesticide resistance.**
...
[Project updates](#updates)
 ...
 - **2022**
   ...
   - **NOV** [24](#u-24-NOV-2022)
   - **JUL** [19](#u-19-JUL-2022), [30](#u-30-JUL-2022)
   - **JUN** [13](#u-13-JUN-2022), [28](#u-28-JUN-2022)
...
> <a name="u-01-MAY-2025">Update: 1 MAY 2025</a>

This package continues to be developed, although I have not written in this notebook much. Since my previous entry, a software note has been published in *PLoS Computational Biology*:

> Duthie, AB, R Mangan, CR McKeon, MC Tinsley, and LF Bussière. 2023. resevol: An R package for spatially explicit models of pesticide resistance given evolving pest genomes. PLoS Computational Biology. 19:e1011691. 10.1371/journal.pcbi.1011691

A new version of resevol 0.3.4.0 was released on CRAN sometime later with a new feature that allowed the user to model crop growth over time. This can make it possible to model crop availability for pests to be quite low until the crop grows over different time steps. Details for how this works are available in the [run_farm_sim](https://bradduthie.github.io/resevol/reference/run_farm_sim.html) documentation. The relevant new arguments to `run_farm_sim` are `crop_growth` and `crop_growth_type`. For `crop_growth`:
...
An explanation of how pesticide thresholds work can be found in a new vignette, [pesticide Thresholds](https://bradduthie.github.io/resevol/articles/pesticide_thresholds.html).

Further development to the package is ongoing with some collaborators applying the package to specific systems, and I hope to apply resevol to address more general evolutionary questions. I will try to update this notebook more frequently. The full history of the project is still version controlled on [GitHub](https://github.com/bradduthie/resevol/).


> <a name="u-24-NOV-2022">Update: 24 NOV 2022</a>
...
> <a name="u-30-JUL-2022">Update: 30 JUL 2022</a>
...
> <a name="u-19-JUL-2022">Update: 19 JUL 2022</a>
...
```{r, echo = FALSE}
change_mat <- matrix(data = c(0, 1, 1, 0), nrow = 2, ncol = 2);
print(change_mat);
```
...
> <a name="u-13-JUN-2022">Update: 13 JUN 2022</a>
...
1. **More explicit crop and pesticide rotation**, as mentioned in [Issue #43](https://github.com/bradduthie/resevol/issues/43). I think that this would be fairly easy to implement and worth the time. Currently, the `run_farm_sim` argument `crop_rotation_type` accepts either 1 or a 2. A 1 results in no rotation of crops on the landscape, and a 2 results in a random transition between types. I think that a custom transition matrix should be possible to insert, whereby each element should specify the probability of transitioning from crop *i* (row) to crop *j* (column). In the simplest case (e.g., could be `crop_rotation_type = 3`), this could just be a binary matrix that forms a cyclical graph. The current options 1 and 2 could even be subsumed in the transition matrix.
