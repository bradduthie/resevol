...
The objective of simulations will be to test how pest population size changes and traits evolve given different crop and pesticide rotation regimes.
...
There will be 6 internal nodes that map loci values to traits (see the Evolutionary Algorithm explanation for details).
...
Hence, the variation in pesticide consumption rate is 0.4 for both pesticides, which is lower than it is for crops.
...
Finally, there is a positive covariance rate of 0.2 between all crop and all pesticide consumption rates.
...
The options used below in `mine_gmatrix` arguments will lead to a computationally intense (and therefore time-consuming) search, particularly due to the large number of networks in the population (`npsize = 12000`), high number of individuals used to test network stress (`indivs = 2000`), high number of hidden nodes (`layers = 6`), and strict stress criteria (`term_cri = -8`).
...
In particular, fewer internal nodes (`layers`) will greatly reduce the time it takes to find a suitable network.
The trade-off against this reduced time is that more layers make it possible to model more highly constrained trait covariances.
Nevertheless, if such constraints are not needed, then only 1-2 `layers` might be necessary.
...
The whole contents of `mg` are then passed on to the `run_farm_sim` function, which initialises pests using the genome values in `mg` before running simulations.
Next, because we want to run a simulation on a customised landscape, we will explain how such a landscape can be generated and inserted into the simulation.
...
In the image above, each colour represents a unique farm, and colours correspond to the numbers within the matrix `simple_terrain` (note that the orientation differs between the matrix and the image).
...
We can show this using the matrix `land_eg`, which is also included in `advanced_techniques_eg.rda`.
The dimensions of the landscape modelled in `land_eg` is 128 by 128 cells, and the landscape includes 14 farms (1-14), grassland (15), forest (16), and water (17).
...
We will run the same simulated population using the `mg` pest genome and `land_eg` landscape [described above](#land) under two different pesticide regimes. 
In both regimes, the same pesticide is applied to farms on each side of the river (e.g., farms 1-7 apply pesticide 1 and farms 8-14 apply pesticide 2).
...
We will then compare output from the two simulations.
...
Finally, we demonstrate how to produce [output](#output_sim1) for simulation 1.
...
For initialisation, a vector elements correspond to farm number and element values correspond to the crop initialised.
...
That is the first 7 farms on one side of the river all start growing crop 1. 
Farms 8-14 instead all start growing crop 2.
...
In the above matrix `rotate_crop_sim1`, matrix elements define the probability of transitioning from one crop type (rows) to another crop type (columns).
Hence, in `rotate_crop_sim1`, farms that have been applying crop 1 will always switch to crop 2, and vice versa.
...
Next, we define pesticide rotation.
...
Mean consumption rate of both crops is set to 1, while mean pesticide consumption rate is set to 0.1.
...
By setting `reproduction_type = "food_based"` and `food_needed_surv = 0`, we model a system in which pests burn the food consumed from ages 0-2 at a rate of 0.5 units per time step after age 3, then die when their reserves drop below 0.
By setting `food_needed_repr = 1`, we ensure that only pests that have 1 or more units of food stored can reproduce.
...
Pests will die if they consume more than 0.1 total units of pesticide (`pesticide_tolerated_surv = 0.1`).
...
Recall that since pests consume food and pesticide only between ages 0-2, then start moving at age 3, they no food or pesticide consumption occurs during movement (if applicable, it could be turned on with `feed_while_moving = TRUE` and `pesticide_while_moving = TRUE`).
...
Crops are rotated every 18 time steps (`crop_rotation_time = 18`), and pesticides are set to rotate every 9 time steps (`pesticide_rotation_time = 9`), but this pesticide rotation has no effect because the same pesticide is applied on rotation (as defined by `rotate_pesticide_sim1`).
This can be conceptualised as modelling a 18 time step growing season in which pesticides are re-applied at the start and halfway point of a season.
...
Lastly, we set the number of time steps to `time_steps = 240`, and we set `pesticide_start = 81`, which means that pesticides are not applied at all until after a burn-in of 120 time steps.
...
We print out the dynamics of the evolving population over time, and all of the individual data from the last time step, to a CSV file (individual data are not included in the package due to its size).
All values not mentioned are set to default parameter values, which can be found in the [package documentation](../reference/run_farm_sim.html).


### <a name = "output_sim1">Simulation 1 output</a>

When running the function `run_farm_sim` above, the population size of the pest will be printed in the R console in each time step.
...
If `get_f_coef = TRUE` in `run_farm_sim`, then mean inbreeding coefficients are also reported.
...
Using this data set, we can examine the distribution of pests on the landscape, and the covariance of evolving pest traits. 
Note that information has been removed from the file below to avoid a very large file size. But in this output "last_time_step.csv", traits always begin in column 102, meaning that individual trait values for traits 1, 2, 3, and 4 will be in columns 102, 103, 104, and 105, respectively. 
...
The code identifies the x-locations and y-locations of each pest `i` in `pop_last_time_step`, and these landscape cells are replaced with the number 18.
...
In the case of the above, pest density is higher on some (e.g., 1, 8, 9, 10, and 15) farms than others (e.g., 3, 12, and 13), and this pest density could be interpreted with respect to individual farm crop and pesticide rotation regimes.
...
To avoid repetition, the code for plots will not be included for simulation 2.
...
Consequently, pesticides have very little effect on the amount of crops consumed (Figure 5a,b), and while there is an initial spike in the consumption of both pesticides (Figure 5c,d), this consumption quickly declines.
...
When no pesticide rotation occurred, the trade-off between consumption rate of pesticides 1 and 2 was reflected in a trait covariance of `r end_trait_covs[3, 4]`.
But when pesticides were rotated, the covariance was instead `r end_trait_covs2[3, 4]`.
...
For example, we might repeat the above [initalisation](#mining) of pest genomes to obtain 20 genomes with similar covariance structure, then run [simulation 1](#sim1) and [simulation 2](#sim2) 20 times to get more robust predictions about pest population dynamics.
...
To develop theory, we can also contrast simulations with more fundamental differences.
